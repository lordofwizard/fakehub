...
mod tests_for_commit {
    ...
    fn test_correct_number_of_commits() {
        ...
        let commit_count = String::from_utf8(output.stdout).unwrap().trim().parse::<usize>().unwrap();
        assert_eq!(commit_count, 6, "Expected 6 commits, found {}, (Initial Commit + Blank Commits", commit_count);
    }
    ...
    fn test_invalid_date_format() {
        ...
        Command::new("git").arg("init").current_dir(repo_str).output().unwrap();
